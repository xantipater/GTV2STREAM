...
> This branch includes **unreleased stabilisation changes** on top of PR #19.
> Published v1.2.0 does not contain them. See [validation and release gates](docs/STABILISATION.md)
> and the [v1.2 updater recovery note](UPDATE.md#existing-v120-installations-use-adb-for-the-first-fixed-update).
...
The service handles Google TV `launcherx` clicks, focus/selection, long clicks and
Home/detail window events. Focus alone does not redirect. In particular:
...
If the title row is not ready, the service retries against a fresh accessibility tree after 250 ms, with a final attempt at 600 ms. It fails closed for UI chrome, metadata-only cards, advertisements, and sponsored cards. Titles are deduplicated within a short event window; no accessibility node is retained across events. Recognised controls and installed launcher-app labels terminate the interaction
instead of entering missing-title fallbacks. Edit mode suppresses ambient capture.
These are bounded heuristics, not a guarantee that every OEM label is recognised. Fail-closed wins over the rare title miss.
...
Titles are also read from the card the user actually focused, rather than only from the launcher's ambient panel text. Google TV exposes grid rows such as "Top picks for you" as position labels (`Column 3`) rather than titles, so a focused card is remembered when it is seen and reused when the click arrives; a card title carries more weight than ambient panel text, and a poll that returns no readable title can no longer discard a title that was just captured. Position labels are not searched as titles. The weaker hero-panel fallback reads
only the active launcher window, with bounded traversal and clipping. Its coordinate
bands remain device-specific; unknown layouts may miss titles.

### Targets

**TV & movies target** (Nuvio by default, or Stremio or WuPlay): a matching title is looked up through TMDB `/3/search/multi`, with explicit parenthesised year metadata preserved when the card supplies it.
...
Nuvio, Stremio, WuPlay and SmartTube use `ACTION_VIEW` with `NEW_TASK|CLEAR_TASK`.
Cobalt uses its separate media-search contract and flags described above. Explicit
component rejection is retried within the selected package; the historical Nuvio
path also permits another registered `nuvio://` handler. URI schemes are not
exclusive ownership guarantees. Third-party warm/cold behaviour needs device tests.
...
closed rather than hijack an unrelated launch. No other app's process is killed.

To keep redirects snappy, recent TMDB matches are kept in a 32-entry in-memory cache keyed by normalised title **and explicit year**; an unknown year is a separate key. Re-selecting the same card within 24 hours launches from the cached match without any network call. The cache is memory-only: nothing is stored on disk, and it resets when the application process restarts.
...
The download **and install** button is offered when the release has an APK attached. It downloads the APK, checks published size, bounded ZIP integrity, and Android package/version metadata, and then hands it to the Android system installer for user confirmation, signature
and compatibility checks. Installer session identity and outcomes are persisted so
Settings does not depend on a surviving Activity callback. End-to-end signed
self-update and process-replacement testing remain release gates. GTV2STREAM never installs anything silently. Progress and outcomes are reported in Settings ("Downloading update… 47%", "Update installed", "Installation cancelled", and so on), and a cancelled or failed install leaves the installed app untouched.
...
3. Open GTV2STREAM, enter the key, and select **Save TMDB key**. The key is
   stored in private preferences and sent to TMDB for lookups; it is never
   committed to source.
...
GTV2STREAM has no project-operated backend, analytics or telemetry. It is not
network-free: film/series title queries and the user-provided API key go to TMDB
over HTTPS. Selected IMDb identifiers or YouTube search titles are passed to the
chosen target app, whose own network/privacy behaviour is outside this project.
Opening Settings can contact GitHub for release metadata; downloading an update
contacts GitHub release infrastructure only after user action. Those services can
observe normal request metadata such as the client's IP address.

Settings and the key are held in private preferences. The stabilisation build
excludes preferences from cloud and device-transfer backup and disables normal
app backup; older v1.2.0 used `allowBackup=true`. The source does not establish
whether any particular older TV actually performed a backup.

Normal release builds omit raw event/title diagnostics. Installing a debug build
opts into payload diagnostics in Android Logcat; redact recommendation titles and
other personal information before sharing logs. No viewing-history database or
project telemetry upload is implemented.
